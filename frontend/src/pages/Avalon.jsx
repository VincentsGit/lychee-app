...
export default function Avalon() {
  ...
  return (
    <Stack spacing={4} sx={{ width: "100%", minWidth: 0, overflowX: "hidden" }}>
      <AnimatedSection>
        <Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems={{ md: "flex-end" }} sx={{ minWidth: 0 }}>
          <Box sx={{ flex: 1, width: "100%", minWidth: 0, maxWidth: { xs: "calc(100vw - 48px)", sm: 760 } }}>
            <Typography variant="h1" color="blog.subheading" sx={wrapSx}>Avalon</Typography>
            <Typography color="text.secondary" sx={wrapSx}>
              Generate secret roles, pass the phone around, then track quests, votes, and the final Merlin guess.
            </Typography>
          </Box>
